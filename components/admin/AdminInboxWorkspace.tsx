...
export function AdminInboxWorkspace() {
  ...
  const load = useCallback(async () => { if (!allowed) return; try { setErrorKind(null); setMessages(await CommunicationCenterService.getAllUserCommunications()); } catch (error) { setErrorKind(classifyCommunicationError(error, typeof navigator === 'undefined' ? true : navigator.onLine)); } }, [allowed]);
