...
export function AstroTodayCard({ context }: AstroTodayCardProps) {
  ...
  const a = translations[language].astroToday;
  ...
  const formatDate = (date: string) => new Intl.DateTimeFormat(language === "en" ? "en-US" : "id-ID", { day: "numeric", month: "long", year: "numeric", timeZone: timezone }).format(new Date(`${date}T12:00:00`));
  const formatInstant = (iso: string) => new Intl.DateTimeFormat(language === "en" ? "en-US" : "id-ID", { day: "numeric", month: "long", year: "numeric", timeZone: timezone }).format(new Date(iso));
  ...
  const westernCards: TimingCard[] = relevantWestern.map((event) => {
    ...
    return {
      ...
      title: `${(a.planets as Record<string, string>)[event.body] || event.body} di ${event.sign}`,
      timing,
  ...
  if (eclipseData.globalNext) {
    ...
    eclipseCards.push({
      ...
      title: `${a.eclipseGlobalNext}: ${isSolar ? "Matahari" : "Bulan"} ${info.subkind}`,
