...
import { useBroadcastHistory } from '@/hooks/useCommunications';
import { formatDateTime } from '@/lib/analytics';

const PREMIUM_PLANS = new Set(['Google Play Paid','Penjaga Inti','Penjaga Alfa','Founder']);
const FREE_PLANS = new Set(['Free','Expired Grant','Expired Paid']);

export function BroadcastPage(){
  const founder=useFounderUsers();
  const comm=useBroadcastHistory();
  const [audience,setAudience]=useState('all');
  const [title,setTitle]=useState('');
  const [content,setContent]=useState('');
  const targetUsers=useMemo(()=>founder.users.filter((u)=>{
    if(audience==='all') return true;
    if(audience==='premium') return PREMIUM_PLANS.has(u.plan);
    if(audience==='trial') return u.plan==='Trial';
    if(audience==='free') return FREE_PLANS.has(u.plan);
    return false;
  }),[founder.users,audience]);
  ...
  return <div className="page">
    ...
    <div className="notice" style={{marginBottom:14}}>Audience mengikuti akses saat ini: Premium hanya Paid + Inti/Alfa aktif + Founder. Free mencakup Free serta grant/paid access yang sudah selesai. Unknown Legacy tidak otomatis dianggap Premium.</div>
    ...
    <div className="grid-2">
      <section className="panel"><div className="panel-head"><div><div className="panel-title">Compose Draft</div><span className="panel-subtitle">Tidak menulis ke Firestore sampai safeguard server-side selesai.</span></div><span className="source-badge warn">DRAFT ONLY</span></div><div className="panel-body">
        <div className="toolbar"><select className="select" value={audience} onChange={e=>setAudience(e.target.value)}><option value="all">All real users</option><option value="premium">Premium access aktif</option><option value="trial">Trial aktif</option><option value="free">Free / expired access</option></select><span className="pill green">{targetUsers.length} recipients</span></div>
        ...
        <div className="notice" style={{marginTop:12}}>Mass-send tetap dikunci sampai recipient resolution, idempotency, Founder verification, dan error reporting server-side selesai.</div>
      </div></section>
