    ...
    <div className="space-y-8">
      <header>
        <h4 className="text-[#4F6658] font-bold text-lg italic">
          {isEn ? "Recommended Support Pathway" : "Rekomendasi Jalur Dukungan"}
        </h4>
        <p className="text-[10px] text-[#7B8776] font-bold uppercase tracking-widest mt-1">
          {isEn ? "Tailored support options" : "Pilihan dukungan yang paling sesuai"}
        </p>
      </header>

      <div className="space-y-4">
        <SupportCard recommendation={primary} isPrimary language={language} isEn={isEn} />
        {secondary && <SupportCard recommendation={secondary} language={language} isEn={isEn} />}
      </div>
    </div>
  );
}

function SupportCard({ recommendation, isPrimary, language, isEn = false }: { recommendation: SupportPathRecommendation; isPrimary?: boolean; language: "id" | "en"; isEn?: boolean }) {
  ...
  const getConfidenceLabel = (conf: string) => {
    ...
    return conf === "HIGH" ? "HIGH" : conf === "MEDIUM" ? "MODERATE" : "LOW";
  };
      ...
      <div className="space-y-4">
        <div>
          <p className="text-[9px] font-bold text-[#9AA394] uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
            <Info size={10} /> {isEn ? "Why this step?" : "Mengapa langkah ini?"}
          </p>
        ...
        {level >= 2 && level <= 4 && (
          <p className="text-[10px] text-[#7B8776] italic font-medium p-3 bg-white/50 rounded-xl border border-[#E8E9E5]/40">
            "{isEn ? SUPPORT_DISCLAIMERS.wellness_en : SUPPORT_DISCLAIMERS.wellness}"
          </p>
        )}

        <div className="space-y-2 pt-2">
           <p className="text-[9px] font-bold text-[#7B8776] uppercase tracking-[0.2em]">
             {isEn ? "Support Options" : "Pilihan Pendampingan"}
           </p>
           <div className="space-y-2">
             {resources.map((res) => (
               <div key={res.id} className="flex items-center justify-between p-3 rounded-xl bg-white border border-[#E8E9E5] hover:border-[#4F5E52]/30 transition-all group cursor-pointer"
                    onClick={() => {
                 ...
                 <div>
                   ...
                   <p className="text-[9px] text-[#7B8776] font-medium">{res.purpose[langKey]}</p>
                 </div>
