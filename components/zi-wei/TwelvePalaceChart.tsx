...
export function TwelvePalaceChart({
  palaces,
  interpretations,
  isEn = false,
}: {
  palaces: ZiWeiPalace[];
  interpretations: ZiWeiSection[];
  isEn?: boolean;
}) {
  return (
      ...
      <div className="grid min-w-[760px] grid-cols-4 grid-rows-4 gap-2" aria-label="Twelve Palace Chart">
        {palaces.map((palace) => {
          ...
          <article key={palace.index} className={`${GRID_POSITIONS[palace.index] ?? ""} min-h-40 rounded-2xl border border-[#E5DDCF] bg-white p-4 shadow-sm`}>
            ...
            <p className="mt-3 text-sm font-semibold leading-5 text-[#4F5E52]">{palace.majorStars.map((star) => star.canonicalName).join(" · ") || (isEn ? "No Major Star" : "Tanpa Major Star")}</p>
            ...
            {interpretation && <details className="group mt-3 border-t border-[#F3EFE6] pt-2">
              <summary className="cursor-pointer list-none text-xs font-bold text-[#4F5E52]">
                <span className="group-open:hidden">{isEn ? "View meaning" : "Lihat makna"}</span>
                <span className="hidden group-open:inline">{isEn ? "Close meaning" : "Tutup makna"}</span>
              </summary>
              <div className="mt-2 space-y-2 text-xs leading-5 text-[#7B8776]">
                ...
                {interpretation.challenge && <p><span className="font-bold text-[#4F5E52]">{isEn ? "Areas for care:" : "Yang perlu dijaga:"}</span> {interpretation.challenge.replace(/^(Yang perlu dijaga adalah|What needs care is)\s*/i, "")}</p>}
                {interpretation.growthDirection && <p><span className="font-bold text-[#4F5E52]">{isEn ? "Growth direction:" : "Arah pertumbuhan:"}</span> {interpretation.growthDirection}</p>}
              </div>
            </details>}
          </article>
          );
        ...
        <div className="col-start-2 col-end-4 row-start-2 row-end-4 flex flex-col items-center justify-center rounded-3xl bg-[#4F5E52] p-6 text-center text-white">
          ...
          <p className="mt-2 max-w-xs text-xs leading-5 text-[#D7DDD5]">{isEn ? "Palaces are interpreted as an interconnected structure." : "Istana dibaca sebagai satu struktur yang saling terhubung."}</p>
        </div>
      </div>
    </div>
  );
