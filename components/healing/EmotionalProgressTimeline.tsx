  ...
  return (
    <section className="rounded-[32px] bg-white p-7 shadow-soft border border-[#E8E9E5]">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-[#7B8776] text-xs uppercase tracking-[0.24em] mb-2">{isEn ? "Healing Journey" : "Perjalanan penyembuhan"}</p>
          <h2 className="text-2xl font-semibold text-[#33413A]">{isEn ? "Emotional progress timeline" : "Garis waktu perkembangan emosional"}</h2>
        </div>
        <p className="text-sm text-[#7B8776]">{memory.healingMilestones.length} {isEn ? "milestones" : "tonggak"}</p>
      </div>
          ...
          <div className="rounded-[24px] bg-[#F7F4ED] p-6 text-[#5F6B60] leading-relaxed">
            {isEn ? (
              <>
                ...
                <p className="mt-2 text-sm text-[#7B8776]">Your first practice today will begin your story of gentle transformation.</p>
              </>
            ) : (
              <>
          ...
          memory.healingMilestones.map((milestone) => (
            <div key={milestone.date} className="relative rounded-[24px] border border-[#E8E9E5] bg-[#FAF7F1] p-6">
              ...
              <div className="ml-6 space-y-2">
                <p className="text-[#7B8776] text-xs uppercase tracking-[0.24em]">
                  {new Date(milestone.date).toLocaleDateString(isEn ? "en-US" : "id-ID", { weekday: "short", day: "numeric", month: "short", year: "numeric" })}
                </p>
                <p className="text-[#4F5E52] font-semibold">{milestone.milestone}</p>
                <p className="text-[#6B7B6C] text-sm leading-relaxed">{isEn ? "Journal ref: " : "Ref jurnal: "}{milestone.journalReference}</p>
              </div>
            </div>
          ))
        )}
      ...
      {memory.recurringThemes.length > 0 && (
        <div className="mt-8 rounded-[24px] bg-[#F4F2EC] p-5 border border-[#E8E9E5]">
          <p className="text-[#7B8776] text-xs uppercase tracking-[0.24em] mb-3">{isEn ? "Recurring themes" : "Tema berulang"}</p>
