...
function NarrativeCard({ card, isEn = false }: { card: WholeSignNarrativeCard; isEn?: boolean }) {
  return (
    <article className="rounded-2xl border border-[#E8E1D3] bg-white p-5 shadow-sm">
      ...
      <details className="group mt-4 border-t border-[#EEE8DD] pt-3">
        <summary className="cursor-pointer list-none text-xs font-bold text-[#7B8776] marker:content-none">
...
export default function WholeSignPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setPresentation(buildWholeSignPresentation(result, { isEn }));
      } catch (error) {
  ...
  if (loading) return <ProtectedRoute><main className="min-h-screen bg-[#FCFAF5] px-5 py-8"><AppNav /><p className="mt-24 text-center text-[#7B8776]">{isEn ? "Arranging your life houses..." : "Menyusun rumah kehidupanmu..."}</p></main></ProtectedRoute>;
  if (!presentation) return <ProtectedRoute><main className="min-h-screen bg-[#FCFAF5] px-5 py-8"><AppNav /><p className="mt-24 text-center text-[#7B8776]">{isEn ? "Whole Sign Birth Chart cannot yet be built from the available data." : "Whole Sign Birth Chart belum dapat dibangun dari data yang tersedia."}</p></main></ProtectedRoute>;
      ...
      <main className="min-h-screen bg-[#FCFAF5] px-5 py-8 pb-32">
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-12 space-y-12">
            ...
            {presentation.ascendant && <NarrativeCard card={presentation.ascendant} isEn={isEn} />}
            <div className="grid gap-4 md:grid-cols-2">
              {presentation.sun && <NarrativeCard card={presentation.sun} isEn={isEn} />}
