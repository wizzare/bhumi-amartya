  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setPresentation(buildAstrocartographyPresentation(calculated, { isEn }));
        setAutomatic(buildAutomaticAstrocartographyPresentation(calculated, { birthCountryCode, isEn }));
      } catch (error) {
  ...
  if (!result || !presentation) {
    return (
      <ProtectedRoute>
        <main className="min-h-screen bg-[#FCFAF5] px-5 py-8">
          <AppNav />
          <p className="mx-auto mt-24 max-w-xl text-center leading-7 text-[#7B8776]">
            {isEn
              ? "Astrocartography map cannot be generated yet. Your birth data remains secure and no approximate lines are displayed."
              : "Peta Astrocartography belum dapat dibangun. Data kelahiranmu tetap aman dan tidak ada garis perkiraan yang ditampilkan."}
          </p>
        </main>
      </ProtectedRoute>
    );
      ...
      <main className="min-h-screen overflow-x-hidden bg-[#FCFAF5] px-5 py-8 pb-32">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <header className="rounded-3xl bg-[#4F5E52] p-6 text-white shadow-md sm:p-9">
            ...
            <p className="mt-2 font-serif text-2xl text-[#D8E0D7]">{presentation.hero.title}</p>
            {presentation.availabilityStatus === "available" && (
              <p className="mt-5 inline-flex rounded-full bg-white/10 px-4 py-2 text-sm">
                <strong>{presentation.hero.lineCount}</strong>&nbsp;{isEn ? "calculated lines" : "garis terhitung"}
              </p>
            )}
            {automatic && (
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="rounded-full bg-white/10 px-4 py-2 text-sm">{isEn ? "Primary theme" : "Tema utama"} · {automatic.strongestCategory}</span>
                <span className="rounded-full bg-white/10 px-4 py-2 text-sm">{automatic.referenceCities.length} {isEn ? "reference regions" : "wilayah referensi"}</span>
              </div>
            )}
            <p className="mt-5 max-w-3xl leading-7 text-[#D8E0D7]">
              {isEn
                ? "This map reveals regions where themes of home, vocation, relationships, and personal growth may feel more pronounced."
                : "Peta ini menunjukkan wilayah tempat tema rumah, karya, relasi, dan pertumbuhanmu dapat terasa lebih menonjol."}
            </p>
          </header>
          ...
          <div className="mt-10 space-y-10">
            {presentation.availabilityStatus === "available" && automatic ? (
              <section>
                <AstrocartographyMap automatic={automatic} isEn={isEn} />
              </section>
            ) : (
              <section className="rounded-2xl border border-[#E8E1D3] bg-white p-6 text-sm leading-7 text-[#7B8776]">
                <h2 className="font-serif text-2xl text-[#4F5E52]">{isEn ? "Map not available yet" : "Peta belum tersedia"}</h2>
                <p className="mt-3">
                  {isEn
                    ? "Astrocartography map cannot be calculated because it requires complete date, time, time zone, and birth location."
                    : "Peta Astrocartography belum dapat dihitung karena membutuhkan tanggal, jam, zona waktu, dan lokasi lahir yang lengkap."}
                </p>
                <Link href="/settings" className="mt-4 inline-flex font-bold text-[#4F5E52] underline">
                  {isEn ? "Complete birth profile" : "Lengkapi profil kelahiran"}
                </Link>
              </section>
            )}

            <section className="rounded-2xl border border-[#E8E1D3] bg-white p-6 shadow-sm">
              <h2 className="font-serif text-2xl text-[#4F5E52]">{isEn ? "How to Read the Map" : "Cara Membaca Peta"}</h2>
            ...
            {automatic && (
              <section className="rounded-2xl border border-[#D9E4DA] bg-[#F1F7F1] p-5 text-sm leading-7 text-[#657568]">
                <p>{automatic.safetyNote}</p>
                <p className="mt-2">{automatic.privacyNotice}</p>
              </section>
            )}
            ...
            <section className="rounded-3xl bg-[#4F5E52] p-6 text-white shadow-md sm:p-8">
              <div className="mb-5 flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold">{isEn ? "Your Blueprint Summary" : "Kesimpulan Dirimu"}</h2>
              </div>
              <div className="space-y-4 text-sm leading-7 text-[#D8E0D7]">
                {(automatic?.summary || presentation.summary).map((paragraph, index) => <p key={index}>{paragraph}</p>)}
              </div>
            </section>
          </div>
        </div>
