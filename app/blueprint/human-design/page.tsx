...
import { getCanonicalHumanDesign, getHumanDesignCanonicalFailureReason } from "@/lib/humandesign/hdAudit";
...
type PageStatus = "loading" | "canonical" | "pending" | "service" | "fallback" | "missing" | "invalid" | "partial" | "error";

const statusCopy: Record<Exclude<PageStatus, "loading" | "canonical">, { title: string; body: string }> = {
  pending: { title: "Perhitungan sedang disiapkan", body: "Human Design belum terverifikasi. Halaman ini tidak menampilkan chart perkiraan sebagai hasil final." },
  service: { title: "Layanan Human Design belum tersedia", body: "Chart belum dapat diverifikasi. Coba lagi nanti; tidak ada data perkiraan yang ditampilkan sebagai hasil final." },
  fallback: { title: "Pembacaanmu masih disiapkan", body: "Kami belum ingin menyimpulkan terlalu cepat. Coba lagi nanti ketika data kelahiranmu sudah dapat dibaca dengan lebih utuh." },
  missing: { title: "Data kelahiran belum lengkap", body: "Lengkapi tanggal, waktu, zona waktu, dan lokasi kelahiran untuk menyiapkan chart yang dapat diverifikasi." },
  invalid: { title: "Pembacaanmu perlu disiapkan ulang", body: "Ada bagian data yang belum cukup jelas untuk diceritakan dengan jujur. Belum ada kesimpulan yang kami paksakan." },
  partial: { title: "Cerita tentang dirimu belum lengkap", body: "Sebagian data sudah ada, tetapi kami menunggu bagian lain agar pembacaanmu tidak terasa setengah-setengah." },
  error: { title: "Human Design gagal dimuat", body: "Terjadi kesalahan saat membaca chart. Silakan kembali ke Profil dan coba lagi." },
};
...
function classifyStatus(value: unknown, hasBirthData: boolean): PageStatus {
  if (!value) return hasBirthData ? "error" : "missing";
  const reason = getHumanDesignCanonicalFailureReason(value);
  if (reason === "canonical") return "canonical";
  if (!hasBirthData) return "missing";
  if (["timeout", "connection_error", "service_unavailable"].includes(reason)) return "service";
  if (reason === "fallback_quality" || reason === "invalid_source") return "fallback";
  if (reason === "missing_engine_version" || reason === "invalid_status") return "invalid";
  if (reason === "missing_type") return "partial";
  return "invalid";
}
...
export default function HumanDesignPage() {
  ...
  useEffect(() => {
    ...
    void storageProvider.getUserBlueprint().then((value) => {
      ...
      setBlueprint(next);
      const profile = next?.input as { birthDate?: string; birthTime?: string; timezone?: string } | undefined;
      setStatus(classifyStatus(next?.humanDesign, Boolean(profile?.birthDate && profile?.birthTime && profile?.timezone)));
    }).catch((error) => {
      if (!active) return;
  ...
  const chart = useMemo(() => getCanonicalHumanDesign(blueprint?.humanDesign) as HumanDesignChart | null, [blueprint]);
    ...
    <ProtectedRoute>
      <main className="min-h-screen bg-[#FCFAF5] px-5 py-8 pb-32">
        ...
        <div className="mx-auto max-w-lg">
          ...
          {status === "loading" && <p className="py-20 text-center text-sm text-[#7B8776]">Memuat blueprint...</p>}
          {status !== "loading" && status !== "canonical" && (
            <section className="rounded-2xl border border-[#E9E4D9] bg-white p-6 shadow-sm">
              ...
              <h2 className="mt-2 font-serif text-2xl text-[#4F5E52]">{statusCopy[status].title}</h2>
              <p className="mt-3 text-sm leading-6 text-[#7B8776]">{statusCopy[status].body}</p>
              {loadError && <p className="mt-3 break-words text-xs text-rose-700">{loadError}</p>}
            </section>
          )}

          {status === "canonical" && chart && (
