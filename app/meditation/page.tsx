...
export default function MeditationPage() {
  ...
  const toggleBodySignal = (signal: string) => {
    const noneSignals = ["No particular sensation", "Tidak ada sensasi khusus"];
    setBodySignals((current) => {
      if (noneSignals.includes(signal)) {
        return current.includes(signal) ? [] : [signal];
      }

      const withoutNone = current.filter((item) => !noneSignals.includes(item));
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-[#FCFAF5] px-6">
        <div className="rounded-3xl bg-white p-8 shadow-xl text-center max-w-md w-full">
          <p className="text-[#4F5E52] text-lg">{isEn ? "Preparing today's meditation space..." : "Menyiapkan ruang meditasi hari ini..."}</p>
  ...
  const defaultName = isEn ? "Soul" : "Jiwa";
  const firstName = typeof profile?.fullName === "string" ? profile.fullName.split(" ")[0] : defaultName;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <section className="bhumi-card p-6">
          ...
          <p className="mt-2 text-[#4F5E52] leading-relaxed">
            {isEn
              ? "Today's practice blends meditation, mudra, gentle body movement, breathwork, and a closing affirmation."
        ...
        <section className="bhumi-card p-6">
          <h2 className="text-xl font-semibold text-[#4F5E52]">{isEn ? "Mudra Guide" : "Panduan Mudra"}</h2>
          {practice.mudra ? (
            <>
              <p className="mt-3 text-sm leading-relaxed text-[#7B8776]">
                {isEn
                  ? "Practice the mudra throughout the meditation unless instructed otherwise."
              ...
              <div className="mt-5 space-y-5">
                <article className="rounded-3xl bg-white/70 p-5 text-[#4F5E52]">
                  ...
                  <div className="mt-4 space-y-4 text-sm leading-relaxed">
                    <div>
                      <p className="font-semibold text-[#4F5E52]">{isEn ? "Purpose:" : "Tujuan:"}</p>
                      <p className="mt-1 text-[#7B8776]">{practice.mudra.benefits}</p>
                    </div>

                    <div>
                      <p className="font-semibold text-[#4F5E52]">{isEn ? "How to Practice:" : "Cara Melakukan:"}</p>
                    ...
                    <div>
                      <p className="font-semibold text-[#4F5E52]">{isEn ? "Duration:" : "Durasi:"}</p>
                    ...
                    <div className="rounded-2xl bg-[#FCFAF5] p-4">
                      <p className="font-semibold text-[#4F5E52]">{isEn ? "Learning Resources" : "Referensi Belajar"}</p>
                      <div className="mt-3 flex flex-col gap-3 sm:flex-row">
                        ...
                        >
                          {isEn ? "View Mudra Image" : "Lihat Gambar Mudra"}
                        </a>
                        <a
                          ...
                          className="rounded-full border border-[#E8E9E5] bg-white px-4 py-3 text-center text-sm font-medium text-[#4F5E52] transition hover:bg-[#F5F1E8]"
                        >
                          {isEn ? "View Video Tutorial" : "Lihat Video Tutorial"}
                        </a>
                      </div>
                    </div>
                  </div>
        ...
        <section className="bhumi-card p-6">
          ...
          <label className="mt-6 block text-sm font-medium text-[#7B8776]" htmlFor="bodyReflection">
            {isEn ? "What did you notice about your body today?" : "Apa yang kamu sadari dari tubuhmu hari ini?"}
          </label>
          <textarea
            id="bodyReflection"
            ...
            placeholder={isEn ? "Write one or two things your body shared with you..." : "Tuliskan satu atau dua hal yang tubuhmu sampaikan..."}
          />
        </section>
