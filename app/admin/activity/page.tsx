...
function classifyUserPremiumSource(user: FounderUser): "GOOGLE_PLAY_PAID" | "FOUNDER_LIFETIME" | "PENJAGA_INTI" | "PENJAGA_ALFA" | "INTERNAL_TRIAL" | "FREE" | "UNKNOWN_LEGACY" {
  ...
  const isFounder = isFounderUser(user.rawUser);
...
export default function AdminActivityPage() {
  ...
  useEffect(() => {
    if (hasAdminAccess) void loadUsersPage(true);
  }, [hasAdminAccess]);

  useEffect(() => {
    if (hasAdminAccess && activeTab === "analytics" && (!hasLoadedAnalytics.current || rangeDates.start || rangeDates.end)) {
      hasLoadedAnalytics.current = true;
      void loadActivityAnalytics();
    }
  }, [activeTab, hasAdminAccess, loadActivityAnalytics, rangeDates.end, rangeDates.start]);
    ...
    return (
      <main className="min-h-screen bg-[#F7F4ED] px-5 py-10 flex items-center justify-center">
        <div className="max-w-md w-full bg-white border border-[#E3E0D7] rounded-lg p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-[#6D786F]">Halaman ini hanya tersedia untuk admin Bhumi.</p>
