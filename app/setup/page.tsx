...
export default function SetupPage() {
  ...
  useEffect(() => {
    if (user && !initializedRef.current) {
      console.log("[SETUP INIT] User:", user.uid);
      setFullName(user.displayName || "");
      setEmail(user.email || "");
      setDebug(prev => ({ ...prev, authUid: user.uid }));
  ...
  const finalizeSetup = async () => {
    ...
    try {
      ...
      await repairOwnerHumanDesign(uid, user.email || "");

      // 6. Scoped Cache
      await storageProvider.saveUserProfile(finalProfile as any);
      // Re-read blueprint after potential repair
      const latestBlueprint = await blueprintRepository.getUserBlueprint(uid);
      if (latestBlueprint) {
        await storageProvider.saveUserBlueprint(latestBlueprint as any);
      }

      // 6. Verify
      const vProfile = await storageProvider.getUserProfile();
      const vBlueprint = await storageProvider.getUserBlueprint();

      if (vProfile?.setupCompleted === true && vBlueprint && vProfile.uid === uid) {
