...
const houseLabel = (house?: number, isEn = isEnlEdition()) => house ? `${isEn ? "House" : "Rumah"} ${house} (${(isEn ? ASTRO_HOUSE_MEANINGS_EN[house]?.title : NATAL_HOUSE_MEANINGS_ID[house]?.title) || (isEn ? "life domain" : "area kehidupan")})` : (isEn ? "domain with incomplete house data" : "area yang belum memiliki data rumah lengkap");
