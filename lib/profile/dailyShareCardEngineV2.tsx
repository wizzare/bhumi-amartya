...
function buildCatatanHariIni(guidance?: DailyGuidance | null, seed?: string) {
  ...
  if (!categories) {
    return {
      ...
      content: FALLBACK_CATATAN,
    };
  }

  // Eligible sections mapping to DailyGuidance categories
  const keys: Array<keyof NonNullable<DailyGuidance["categories"]>> = [
    "general", "mental", "finance", "love", "relational", "spiritual", "challenges", "opportunities",
  ];

  // Filter only those that exist in guidance and have an insight
  const availableKeys = keys.filter((key) => categories[key] && categories[key].insight && categories[key].insight.trim());
  ...
  // Use seed to pick one section
  const pickedKey = pickDaily(availableKeys, `${seed}:catatan`, availableKeys[0]);
  const pickedCategory = categories[pickedKey];

  return {
    label: "Catatan Hari Ini dari Bhumi",
    content: snippet(pickedCategory.insight, FALLBACK_CATATAN, 2),
  };
}
...
function buildManifestation(manifestation: DailyManifestation | null | undefined, seed?: string) {
  if (!manifestation) {
    return {
      label: "Manifestasi Hari Ini",
      content: FALLBACK_MANIFESTATION,
    };
  }

  const options = [
    { title: "Law of Affirmation", text: manifestation.affirmation },
    { title: "Law of Assumption", text: manifestation.assumption },
    { title: "Law of Attraction", text: manifestation.attraction },
  ...
  const picked = pickDaily(options, `${seed}:manifestation`, options[0]);

  return {
    label: "Manifestasi Hari Ini",
    content: `${picked.title}: ${snippet(picked.text, "", 2)}`,
  };
}
...
,
  ...
  const compositeSeed = `${userSeed}:${dateKey}`;

  return {
    dateKey,
    reflection: buildReflection(guidance),
    catatanHariIni: buildCatatanHariIni(guidance, compositeSeed),
    profileInsight,
    manifestation: buildManifestation(guidance?.manifestation, compositeSeed),
