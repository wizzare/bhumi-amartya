...
import type { JourneyDailyMemory, JourneyDailyRecord, JourneyPracticeResult, JourneyUnifiedEvent } from "@/lib/types/journeyDailyRecord";

import { longitudinalWellnessRepository } from "./longitudinalWellnessRepository";
import { journalRepository } from "./journalRepository";
import { dailyGuidanceRepository } from "./dailyGuidanceRepository";
...
export const journeyRepository = {
  ...
  async getUnifiedTimeline(uid: string, options?: { dateFrom?: string; dateTo?: string; limit?: number; categories?: string[] }): Promise<JourneyUnifiedEvent[]> {
    const limitCount = options?.limit || 90;
    const [states, records, wellnessDoc, journals, weekly] = await Promise.all([
      this.getRecentDailyStates(uid, limitCount),
      this.getRecentDailyRecords(uid, limitCount),
      longitudinalWellnessRepository.get(uid),
      journalRepository.getJournalEntries(uid, limitCount),
      reflectionRepository.getRecentWeeklyReflections(uid, Math.ceil(limitCount / 7))
    ]);

    const events: JourneyUnifiedEvent[] = [];

    // 1. Wellness V4
    (wellnessDoc.events || []).forEach(e => {
      events.push({
        id: e.eventId,
        originId: e.eventId,
        originSource: "wellness_v4",
        dateKey: e.dateKey,
        timestamp: e.timestamp,
        title: e.subcategory || "Praktik",
        category: "Self Care",
        significance: "ordinary",
        originVersion: "v4",
        rawReference: {
          repository: "longitudinalWellness",
          originId: e.eventId,
          type: "WellnessEvent",
          timestamp: e.timestamp,
          period: e.period
    ...
    // 2. Legacy Wellness
    records.forEach(r => {
      (r.practiceResults || []).forEach(p => {
        const source = p.source === "wellness_section_4" ? "wellness_practice" : "journey_practice";
        events.push({
          id: `${r.date}_${p.practiceId}_${p.source || "legacy"}`,
          originId: p.practiceId,
          originSource: source,
          dateKey: r.date,
          timestamp: p.completedAt || r.date,
          title: p.practiceTitle,
          category: p.zone === "A" ? "Reflection" : "Self Care",
          significance: p.reflectionResult ? "meaningful" : "ordinary",
          originVersion: "legacy",
          rawReference: {
            repository: "journeyDailyRecords",
            originId: p.practiceId,
            type: "JourneyPracticeResult",
            timestamp: p.completedAt || r.date
          }
        });
      });
    ...
    journals.forEach(j => {
      events.push({
        ...
        dateKey: j.dateCreated.slice(0, 10),
        timestamp: j.dateCreated,
        title: "Menulis Jurnal",
        summary: "Menulis refleksi pribadi.",
        category: "Reflection",
    ...
    // 4. Daily Check-In
    states.forEach(s => {
      const hasContext = (s.wellnessSnapshot?.lifeSituation?.length || 0) > 0;
      const hasNote = Boolean(s.innerworkReflection);
      if (hasContext || hasNote || s.moodLevel !== undefined) {
        events.push({
          id: `${s.date}_checkin`,
          originId: s.date,
          originSource: "checkin",
          dateKey: s.date,
          timestamp: s.updatedAt || s.date,
          title: "Kabar Hari Ini",
          summary: s.emotionalWord || s.dailySummary,
          category: "Heart",
          significance: (hasContext || hasNote) ? "meaningful" : "ordinary",
          originVersion: "v4",
          rawReference: {
            repository: "dailyStates",
            originId: s.date,
            type: "DailyState",
            timestamp: s.updatedAt || s.date
          }
    ...
    weekly.forEach(w => {
      events.push({
        id: `${uid}_${w.weekId}`,
        originId: w.weekId,
        originSource: "weekly_reflection",
        dateKey: w.endDate,
        timestamp: w.generatedAt,
    ...
    const seen = new Set();
    const deduped: JourneyUnifiedEvent[] = [];

    events.sort((a, b) => {
      if (a.originVersion === "v4" && b.originVersion === "legacy") return -1;
      if (a.originVersion === "legacy" && b.originVersion === "v4") return 1;
      return 0;
    }).forEach(e => {
      const key1 = e.id;
      const key2 = `${e.originSource}_${e.originId}`;
      const key3 = `${e.dateKey}_${e.originId}_${e.originSource}`;
      const period = e.rawReference?.period || "unknown";
      const key4 = `${e.dateKey}_${e.title.toLowerCase()}_${period}`;

      if (!seen.has(key1) && !seen.has(key2) && !seen.has(key3) && !seen.has(key4)) {
        deduped.push(e);
        seen.add(key1);
        seen.add(key2);
        seen.add(key3);
