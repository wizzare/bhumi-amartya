...
export function getTimeWindow(date: Date = new Date()): TimeWindow {
  ...
  if (hour >= 0 && hour <= 4) return "afterMidnight";
...
export function getTimeOfDayGreeting(date: Date = new Date(), language: GreetingLanguage = "id"): string {
  ...
  if (window === "afterMidnight") return "Hai...";
...
export function getTimeAwareGreeting(
  ...
  if (window === "afterMidnight") {
    return `Hai ${firstName}... kamu masih terjaga ya? Semoga semuanya baik-baik saja.`;
  }
...
export function getTimeAwareClosing(
  ...
  if (window === "afterMidnight") return "Tidak perlu melakukan apa-apa sekarang. Tarik napas pelan-pelan saja, itu sudah cukup.";
