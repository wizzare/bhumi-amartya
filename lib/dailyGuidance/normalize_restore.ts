...
function cleanupUserFacingSurface(value: string): string {
  let hariIniCount = 0;
  return value
    .replace(/[AƒA¢A¢â?sA¬A.â?oAƒA¢A¢â?sA¬A,A?"]/g, "")
    .replace(/([.!?]){2,}/g, "$1")
    .replace(/\s+([,.!?;:])/g, "$1")
...
const BAD_ADVICE_PATTERN = /ini selaras dengan|pesan harianmu|inti dirimu|kamu berada di|berdasarkan|["AƒA¢A¢â?sA¬A.â?oAƒA¢A¢â?sA¬A,A?]/i;
...
export function standardizeSoulReflection(
  ...
  const expectedOpening = `Halo, ${firstName}. ${getTimeOfDayGreeting(now, language)}.`;
  ...
  const closing = getTimeAwareClosing(new Date(), language);
  const signOff = language === "en" ? "Warm hugs from Bhumi." : "Peluk hangat dari Bhumi.";

  return cleanupUserFacingSurface(`${expectedOpening}\n\n${cleanBody}\n\n${signOff}`);
}
