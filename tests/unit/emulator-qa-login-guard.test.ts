...
test("only explicit emulator mode is allowed", source.includes('useAuthEmulator !== undefined') && source.includes('useFirebaseEmulators === "true"'));
test("product-specific auth flag supported", source.includes('NEXT_PUBLIC_USE_AUTH_EMULATOR'));
test("local google-qa mode hides QA login", !shouldShow({ nodeEnv: "development", useFirebaseEmulators: "false", useAuthEmulator: "false", enableAndroidQaLogin: undefined, isNativePlatform: false, platform: "web" }));
test("global emulator mode still shows QA login for web dev", shouldShow({ nodeEnv: "development", useFirebaseEmulators: "true", useAuthEmulator: undefined, enableAndroidQaLogin: undefined, isNativePlatform: false, platform: "web" }));
