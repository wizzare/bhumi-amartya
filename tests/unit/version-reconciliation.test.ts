...
console.log("Running Build 109 version reconciliation tests\n");
...
test("APP_VERSION is canonical 5.0.9", APP_VERSION === "5.0.9", `got "${APP_VERSION}"`);
test("APP_VERSION derives from CURRENT_VERSION_NAME", APP_VERSION === CURRENT_VERSION_NAME);
test(
  "RELEASE_NAME is canonical V5 Build 109",
  RELEASE_NAME === "BHUMI AMARTYA V5 BUILD 109",
  `got "${RELEASE_NAME}"`,
);
test(
...
test("active display is not Build 107", !`${APP_VERSION} ${RELEASE_NAME}`.includes("5.0.7 BHUMI AMARTYA V5 BUILD 107"));
test("active display is not Build 108", !`${APP_VERSION} ${RELEASE_NAME}`.includes("5.0.8 BHUMI AMARTYA V5 BUILD 108"));
test(
...
// 2. Canonical BuildInfo is Build 109.
test("CURRENT_VERSION_NAME is 5.0.9", CURRENT_VERSION_NAME === "5.0.9");
test("CURRENT_VERSION_CODE is 109", CURRENT_VERSION_CODE === 109);
test("CURRENT_BUILD_NUMBER is 109", CURRENT_BUILD_NUMBER === "109");

// 3. Android release metadata is Build 109 and matches BuildInfo.
...
test("android versionName is 5.0.9", androidVersionName === "5.0.9", `got "${androidVersionName}"`);
test("android versionCode is 109", androidVersionCode === "109", `got "${androidVersionCode}"`);
