...
export function decision(state: string, expiry?: string) {
...
export async function persistEntitlement(uid: string, purchaseToken: string, state: string, result: ReturnType<typeof decision>) {
  const db = adminDb();
  ...
  await db.runTransaction(async (transaction) => {
    ...
    transaction.set(tokenRef, { uid, provider: "google_play", packageName: process.env.ANDROID_PACKAGE_NAME || "com.bhumiamartya.app", productId: process.env.GOOGLE_PLAY_PRODUCT_ID || "bhumi_premium_monthly", tokenHash: hash, subscriptionState: state, entitlementStatus: result.status, accessUntil: until, lastVerifiedAt: now }, { merge: true });
