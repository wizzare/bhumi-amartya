...
const persistIndex = route.indexOf("await persistEntitlement");
...
if (!route.includes("try {\n        await acknowledgeSubscription") || !route.includes("acknowledgementDeferred = true")) throw new Error("acknowledgement failures must not undo persisted entitlement");
